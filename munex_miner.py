...
class PCMiner:
    ...
    async def navigate(self):
        ...
        while self.round_active and self.current != self.goal and time.time() < self.deadline:
            ...
            progress = min(1.0, self.steps / max(1, self.max_path_length))
            bar_width = 20
            filled = int(progress * bar_width)
            print(f"\r{CYAN}submit knowledge{RESET} nonce={step:4d} "
                f"[{GREEN}{'#' * filled}{RESET}{'.' * (bar_width - filled)}] "
                f"{progress * 100:5.1f}%", end="", flush=True)
            ...
            try:
                ...
                if ack.get("success"):
                    ...
                    filled = int(self.steps / max(1, self.max_path_length) * 20)
                    print(f"\r{BOLD}submit knowledge{RESET} nonce={step:4d} "
                          f"[{GREEN}{'#' * filled}{RESET}{'.' * (20 - filled)}] "
                          f"{self.steps / max(1, self.max_path_length) * 100:5.1f}% "
