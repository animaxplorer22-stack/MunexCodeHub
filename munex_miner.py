...
class MobileMiner:
    ...
    def _find_path_step(self, start, goal, remaining):
        """
        Returns the direction index of the first step on a path that reaches goal
        within `remaining` steps, or None if no such path exists.
        """
        if start == goal:
            return None
        sx, sy, sz = start
        ...
        # BFS that stops when we run out of remaining steps
        q = deque([(sx, sy, sz, [])])
        visited = {start}
        while q:
            x, y, z, path = q.popleft()
            # If we've already used more steps than allowed, skip
            ...
            for idx, (dx, dy, dz) in enumerate(DIR_OFFSETS):
                ...
                if (nx, ny, nz) == (gx, gy, gz):
                    # Found goal: return first step of path
                    if not new_path:
                        return None
                    first = new_path[0]
        ...
        # No path found within remaining steps
        return None
    ...
    async def navigate(self):
        ...
        while self.round_active and pos != self.goal and time.time() < self.deadline:
            # Sync state periodically (goal may move)
            ...
            if dir_idx is None:
                # No path within remaining steps – wait and sync (goal may have moved)
            ...
            try:
                ...
                # Always update last_move_time to pace correctly
                self._last_move_time = time.time()
                ...
                if success:
                    ...
                    if "wall" in str(msg).lower():
                        ...
                        # already updated _last_move_time, so we're fine
                        pass
