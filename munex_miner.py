...
class MobileMiner:
    ...
    def _a_star_step(self, start, goal, remaining_steps):
        size = self.size
        maze = self.maze
        sx, sy, sz = start
        gx, gy, gz = goal
    ...
    async def navigate(self):
        ...
        # Set last move time to now to avoid "too fast" on first move
        ...
        while self.round_active and pos != self.goal and time.time() < self.deadline:
            ...
            dir_idx = self._a_star_step(pos, self.goal, remaining)
            if dir_idx < 0:
                # No path within remaining steps; try to move closer to goal without exceeding
                # Fallback: take first step towards goal ignoring remaining, but risk rejection.
                # Instead, just wait for goal to move or sync.
                print(f"{YELLOW}No path within remaining steps, waiting for goal update...{RESET}")
            ...
            try:
                ...
                if success:
                    ...
                    # Update local wall map if wall hit
                    if "wall" in str(msg).lower():
                        dx, dy, dz = DIR_OFFSETS[dir_idx]
                        self.local_walls.add((pos[0] + dx, pos[1] + dy, pos[2] + dz))
                    # If "path too long", maybe goal moved; sync state
                    if "path too long" in str(msg).lower():
                        print(f"{YELLOW}Path too long, syncing state...{RESET}")
                        await self.sync_state()
                    # If step mismatch, sync
                    if "step mismatch" in str(msg).lower():
                        print(f"{YELLOW}Step mismatch, syncing state...{RESET}")
                        await self.sync_state()
                    # If too fast, ensure we wait longer
                    if "too fast" in str(msg).lower():
                        self._last_move_time = time.time()  # reset to force wait
                    # If knowledge issue, resubmit
