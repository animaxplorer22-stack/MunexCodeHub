...
logger = logging.getLogger("PC_Miner")
...
{CYAN}{BOLD}
  ...
  ██║     ╚██████╔╝    ██║ ╚═╝ ██║██║██║ ╚████║███████╗██║  ██║
  ╚═╝      ╚═════╝     ╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝
{RESET}{CYAN}{BOLD}  ─── PC MINER – MAINNET ───{RESET}
"""
...
POW_DIFFICULTY = 16          # 16 bits of leading zeros
...
def a_star(maze, start, goal, known_walls):
    """A* search on 3D grid (z,y,x) order: maze[z][y][x]."""
    size = len(maze)
    sx, sy, sz = start
    gx, gy, gz = goal
...
def solve_pow(seed: str, difficulty: int) -> int:
    ...
    nonce = 0
    # We need to compare the first 'difficulty' bits of the hash to zero.
    # For difficulty <= 256, we can check if int.from_bytes(digest[:4], 'big') is below a threshold.
    # For 16 bits, threshold = 2^(256-16) = 2^240. We'll simply check first 16 bits.
    # More efficient: check first 2 bytes (16 bits) == 0.
    # For difficulty > 16, we need more bytes. We'll just do general check.
    # For simplicity: digest = SHA256(seed + str(nonce)); check if int.from_bytes(digest, 'big') >> (256 - difficulty) == 0.
    while True:
        data = f"{seed}{nonce}".encode()
        digest = hashlib.sha256(data).digest()
        # Check if first 'difficulty' bits are zero:
        # Convert first (difficulty // 8 + 1) bytes to int, mask.
        # For simplicity, we'll just use int.from_bytes(digest, 'big') >> (256 - difficulty) == 0.
        # This is slower but works for small difficulty.
...
class PCMiner:
    def __init__(self, wallet, privkey, miner_id=None, node_url=None):
        ...
        # PoW pending (when we hit a wall, we solve and resume)
        self.pow_pending = False
        self.pow_future = None
    ...
    async def solve_pow_async(self, seed, difficulty):
        """Solve PoW in a thread to avoid blocking the event loop."""
    ...
    async def navigate(self):
        ...
        # Initial knowledge report
        await self.report_walls(force=True)

        logger.info(f"Block {self.block_id}: start {self.start}, goal {self.goal}, deadline {self.deadline}")

        while self.round_active and self.current != self.goal and time.time() < self.deadline:
            # If we are in a PoW state, wait until solved
            if self.pow_pending:
            ...
            try:
                ...
                # Check for wall_hit (node returns a special wall_hit message)
                if ack.get("type") == "wall_hit":
                    seed = ack["seed"]
                    difficulty = ack.get("difficulty", POW_DIFFICULTY)
                    logger.info(f"Wall hit! Solving PoW...")
                ...
                if ack.get("success"):
                    ...
                    print(f"\r{BOLD}[{self.steps:3d}/{self.max_steps:3d}]{RESET} "
    ...
    async def handle_message(self, data):
        ...
        if msg_type == "maze_init":
            ...
            # This is sent as a separate message? Actually the node returns it as response to maze_move.
            # But we also handle it directly in navigate.
            # We'll just forward to pending ack if any.
            if self.pending_ack and not self.pending_ack.done():
                self.pending_ack.set_result(data)
