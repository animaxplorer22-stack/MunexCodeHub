...
class MobileMiner:
    ...
    def _best_step(self, pos, goal, remaining):
        """
        Returns index of best direction that minimises estimated distance to goal,
        considering remaining steps. If a full path exists within remaining steps,
        returns first step of that path. Otherwise, returns the neighbour that
        gets closest to goal (Manhattan distance) and is within remaining steps.
        """
        sx, sy, sz = pos
        gx, gy, gz = goal
        size = self.size
        ...
        # Simple greedy: try each direction, compute Manhattan distance to goal
        best_idx = 0
        best_score = float('inf')
        for i, (dx, dy, dz) in enumerate(DIR_OFFSETS):
            ...
            if (nx, ny, nz) in self.local_walls:
                continue
            # Manhattan distance
            dist = abs(nx - gx) + abs(ny - gy) + abs(nz - gz)
            if dist < best_score:
                best_score = dist
                best_idx = i

        # Now verify if a full path exists within remaining steps using BFS
        q = deque([(sx, sy, sz, [])])
        visited = {pos}
        while q:
        ...
        # No full path within remaining, return the greedy best step
        return best_idx
    ...
    async def navigate(self):
        ...
        # Periodic state sync to keep up with goal movement
        last_sync = 0

        while self.round_active and pos != self.goal and time.time() < self.deadline:
            # Sync state every 10 seconds to get updated goal
            ...
            dir_idx = self._best_step(pos, self.goal, remaining)
            # dir_idx is always valid (at least one open neighbour)
            ...
            try:
                ...
                if success:
                    ...
                    if "path too long" in str(msg).lower() or "step mismatch" in str(msg).lower():
                        await self.sync_state()
