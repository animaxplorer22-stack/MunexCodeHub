...
class MobileMiner:
    ...
    async def submit_knowledge(self):
        ...
        try:
            ...
            # Wait for acknowledgment (handled via message loop)
            ack = await asyncio.wait_for(self._wait_for_knowledge_ack(), timeout=KNOWLEDGE_ACK_TIMEOUT)
            self._knowledge_pending = False
    ...
    async def _wait_for_knowledge_ack(self):
        if not hasattr(self, '_knowledge_future'):
            self._knowledge_future = None
        self._knowledge_future = asyncio.get_event_loop().create_future()
        return await self._knowledge_future
    ...
    async def navigate(self):
        ...
        # Submit initial knowledge and wait for confirmation
        ...
        while self.round_active and pos != self.goal and time.time() < self.deadline:
            ...
            # Ensure knowledge is fresh (resubmit if needed)
            ...
            # Respect min_step_interval
            wait_time = self.step_interval - (time.time() - self._last_move_time)
            if wait_time > 0:
            ...
            try:
                ...
                if success:
                    ...
                    # After successful move, refresh knowledge periodically
                    ...
                    # If rejected due to knowledge, resubmit
                    if "knowledge" in str(msg).lower():
                        print(f"{YELLOW}Knowledge issue, resubmitting...{RESET}")
                        await self.submit_knowledge()
                    # If step mismatch, sync state and retry
                    if "step mismatch" in str(msg).lower():
                        print(f"{YELLOW}Step mismatch detected, syncing state...{RESET}")
                        await self.sync_state()
                        # After sync, steps_taken should be updated, so we continue loop without increment

            except asyncio.TimeoutError:
                print(f"{RED}[Step {step:03d}] TIMEOUT - syncing state...{RESET}")
                logger.warning("Move ack timeout")
                # Sync state to recover
                await self.sync_state()
                # After sync, we might have advanced steps, so continue loop without retrying same step
    ...
    async def handle_message(self, data):
        ...
        if msg_type == "maze_init":
            ...
            self.step_interval = data.get("step_interval", 1.0)
            ...
            if hasattr(self, '_knowledge_future') and self._knowledge_future and not self._knowledge_future.done():
