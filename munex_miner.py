...
class MobileMiner:
    ...
    async def register(self):
        ...
        # If the node responds with immediate registration (no memory challenge)
        ...
        if data.get("type") == "memory_challenge_request":
            ...
            # Wait for final response (registered or maze_init)
            ...
            if final_data.get("type") == "maze_init":
                ...
                print(f"{RED}Registration failed after challenge: unexpected response {final_data.get('type')}{RESET}")
                return False

        print(f"{RED}Unexpected registration response: {data.get('type')}{RESET}")
        return False
    ...
    async def handle_message(self, data):
        ...
        if msg_type == "maze_init":
            ...
            self.step_interval = data.get("step_interval", 0.5)
