...
{CYAN}{BOLD}
 ...
 ██║  ██║██║     ███████╗██╔╝ ██╗    ██║ ╚═╝ ██║██║██║ ╚████║███████╗██║  ██║
 ╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝    ╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝
{RESET}{CYAN}{BOLD}  ─── APEX MOBILE MINER – LIGHT & FAST ───{RESET}
"""
...
# Hashstep
# -----------------------------------------------------------------------------
def hashstep_chain_bytes(message: str, depth: int) -> bytes:
    h = hashlib.sha256(message.encode()).digest()
    for _ in range(depth - 1):
        h = hashlib.sha256(h).digest()
    return h

def nonce_satisfies(nonce: int, prefix: str, target_bytes: bytes, depth: int) -> bool:
    return hashstep_chain_bytes(f"{prefix}{nonce}", depth) < target_bytes
...
class MobileMiner:
    def __init__(self, wallet=None, privkey=None, miner_id=None, node_url=None):
        ...
        self.block_id = -1
        self.deadline = 0
        self.hashstep_depth = DEFAULT_HASHSTEP_DEPTH
        self.target = (1 << 256) - 1
        self.target_bytes = self.target.to_bytes(32, "big")
        self.difficulty = 1.0
    ...
    def _print_block_start(self):
        ...
        print(f"{CYAN}Size: {size}x{size}x{size} | Target: {goal} | "
              f"Difficulty: {diff} | Depth: {depth}{RESET}\n")
    ...
    # -------------------------------------------------------------------------
    # Balance request – FIXED: no .open attribute
    # -------------------------------------------------------------------------
    ...
    async def register(self):
        ...
        if data.get("type") == "registered":
            ...
            if "difficulty" in data:
                ...
                self.target = (1 << 256) - 1
                self.target = self.target // int(2 * self.difficulty)
                self.target_bytes = self.target.to_bytes(32, "big")
            bal_mcx = self.balance // 1_000_000
            print(f"{GREEN}✅ REGISTERED! Balance: {bal_mcx:,} MCX | Diff: {self.difficulty}{RESET}")
            # Non-blocking balance request
    ...
    async def _find_nonce(self, prefix: str) -> int:
        nonce = 0
        while True:
            if nonce_satisfies(nonce, prefix, self.target_bytes, self.hashstep_depth):
                return nonce
            nonce += 1
            if nonce > 0xFFFFFFFF:
    ...
    async def navigate(self):
        ...
        while self.round_active and pos != self.goal and time.time() < self.deadline:
            ...
            nonce = await self._find_nonce(prefix)
    ...
    async def handle_message(self, data):
        ...
        if msg_type == "maze_init":
            ...
            self.deadline = data["deadline"]
            self.hashstep_depth = data.get("hashstep_depth", 400)
            ...
            # Skip if deadline expired
            if self.deadline <= time.time() + 2:
                print(f"{YELLOW}⚠️ Block #{self.block_id} already expired (deadline: {self.deadline:.1f}, now: {time.time():.1f}). Waiting for next...{RESET}")
                self.round_active = False
                return
            ...
            if data.get("success"):
                ...
                if "wall" in str(data.get("message", "")).lower():
                    pass
